...
//Made with help from: https://towardsdatascience.com/understanding-and-implementing-neural-networks-in-java-from-scratch-61421bb6352c
public class Matrix {
    private double[][] data;
    ...
    /**
     * Puts each number in this matrix through a sigmoid function.
     * @see MathUtils#sigmoid(double)
     * */
    public void sigmoid() {
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                this.data[i][j] = MathUtils.sigmoid(this.data[i][j]);
            }
        }
    }

    /**
     * Returns a new matrix that went through the dsigmoid function.
     * @return a new matrix that went through the dsigmoid function.
     * @see MathUtils#dsigmoid(double)
     * @see Matrix#sigmoid()
     * @return a new matrix that went through the dsigmoid function.
     * */
    public Matrix dsigmoid() {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                temp.set(i, j, MathUtils.dsigmoid(this.data[i][j]));
            }
        }
